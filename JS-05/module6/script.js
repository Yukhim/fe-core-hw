...
class Hamburger {
  ...
  removeTopping(topping) {
    for(const key in this._toppings){
      if(this._toppings[key]== topping){
        this._toppings.splice(key,1);
 ...
 get getSize() {
   ...
   */
  get calculatePrice() {
    const toppingPrice = this._toppings.reduce((accum, item)=>accum+Hamburger.TOPPINGS[item].price, 0);
  ...
  get calculateCalories() {
    const toppingCalories = this._toppings.reduce((accum, item)=>accum+Hamburger.TOPPINGS[item].calories, 0);
